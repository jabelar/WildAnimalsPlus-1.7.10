...
public class GuiContainerCreative extends InventoryEffectRenderer
{
    ...
    protected void renderToolTip(ItemStack itemIn, int x, int y)
    {
        if (selectedTabIndex == CreativeTabs.tabAllSearch.getTabIndex())
        {
            ...
            CreativeTabs creativetabs = itemIn.getItem().getCreativeTab();

            if (creativetabs == null && itemIn.getItem() == Items.enchanted_book)
            {
                Map map = EnchantmentHelper.getEnchantments(itemIn);
            ...
            for (int i1 = 0; i1 < list.size(); ++i1)
            {
                if (i1 == 0)
                {
                    list.set(i1, itemIn.getRarity().rarityColor + (String)list.get(i1));
                }
                else
                {
